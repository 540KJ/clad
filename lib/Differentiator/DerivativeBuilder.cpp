...
namespace clad {
  ...
  DeclWithContext DerivativeBuilder::Derive(const FunctionDecl* FD,
    ...
    if (request.Mode == DiffMode::forward) {
      ...
      int numIndependentArgs = request.CallContext->getNumArgs() - 1;

      HessianModeVisitor H(*this);
      std::vector<FunctionDecl*> FunctionSave;

      for (int i = 1; i <= numIndependentArgs; i++)
      {
        DiffRequest independentArgRequest = request;
        independentArgRequest.Args = request.CallContext->getArg(i);
        independentArgRequest.Mode = DiffMode::forward;
        independentArgRequest.CallUpdateRequired = false;
        independentArgRequest.VerboseDiags = false;

        FunctionDecl* firstDerivative = plugin::ProcessDiffRequest(m_CladPlugin, independentArgRequest);
        printf("ha\n");
        ...
        independentArgRequest.CallUpdateRequired = false;
        FunctionDecl* secondDerivative = plugin::ProcessDiffRequest(m_CladPlugin, independentArgRequest);
        printf("%s\n", secondDerivative->getNameAsString().c_str());

        // Stmt* reverseBody = reversePass->getBody();
        // reverseBody->dumpPretty(m_Context);
        FunctionSave.push_back(secondDerivative);
      }
      result = H.Merge(FunctionSave, request);
    }
  ...
  DeclWithContext HessianModeVisitor::Merge(std::vector<FunctionDecl*> Functions, const DiffRequest& request) {
    llvm::SmallVector<const clang::VarDecl*, 16> args;
    std::copy(request.Function->param_begin(), request.Function->param_end(), std::back_inserter(args));
    ...
    llvm::SmallVector<QualType, 16> paramTypes(request.Function->getNumParams() + 1);
    std::transform(request.Function->param_begin(),
    request.Function->param_end(),
    std::begin(paramTypes),
    [] (const ParmVarDecl* PVD) {
      return PVD->getType();
    });
    // paramTypes.back() = m_Context.getPointerType(m_Context.getPointerType(request.Function->getReturnType()));
    auto size_type_bits = m_Context.getIntWidth(m_Context.getSizeType());
    paramTypes.back() = m_Context.getConstantArrayType(request.Function->getReturnType(), llvm::APInt(size_type_bits, request.Function->getNumParams()), ArrayType::ArraySizeModifier::Normal,
                                       0);
    paramTypes.back() = m_Context.getConstantArrayType(paramTypes.back(), llvm::APInt(size_type_bits, request.Function->getNumParams()), ArrayType::ArraySizeModifier::Normal,
                                       0);

    // ArrayType::ArrayType(m_Context.getPointerType(request.Function->getReturnType()), request.Function->getType(), request.Function->getType())
    ...
    DeclContext* DC = const_cast<DeclContext*>(request.Function->getDeclContext());
    ...
    if (isa<FunctionDecl>(request.Function)) {
      ...
      hessianFD = FunctionDecl::Create(m_Context, m_Sema.CurContext, noLoc,
        ...
        request.Function->getTypeSourceInfo(),
        request.Function->getStorageClass(),
        request.Function->isInlineSpecified(),
        request.Function->hasWrittenPrototype(),
    ...
    std::transform(request.Function->param_begin(), request.Function->param_end(),
    std::begin(params),
    [&] (const ParmVarDecl* PVD) {
    ...
    params.back() = ParmVarDecl::Create(m_Context, hessianFD, noLoc,
        noLoc, &m_Context.Idents.get("_2dresult"),
        paramTypes.back(),
    ...
    {
      Expr* exprFunc = BuildDeclRef(Functions[i]);

      auto size_type = m_Context.getSizeType();
      auto size_type_bits = m_Context.getIntWidth(size_type);
      // Create the idx literal.
      auto j = IntegerLiteral::Create(m_Context, llvm::APInt(size_type_bits, i),
      size_type, noLoc);
      // Create the _result[idx] expression.
      auto result_at_j = m_Sema.CreateBuiltinArraySubscriptExpr(m_Result, noLoc,
        j, noLoc).get();

      // result_at_j->dump();

      std::vector<Expr*> paramResult;
      paramResult.resize(params.size());
      std::transform(params.begin(), std::prev(params.end()),
      std::begin(paramResult),
      [&] (ParmVarDecl* PVD) {
        // PVD->dump();
      ...
      paramResult.pop_back();
      paramResult.push_back(result_at_j);

      Expr* call = m_Sema.ActOnCallExpr(getCurrentScope(), exprFunc, noLoc, llvm::MutableArrayRef<Expr*>(paramResult), noLoc).get();
      // call->dump();
      // printf("yay\n");
    ...
    auto Stmts_ref = llvm::makeArrayRef(CompStmtSave.data(), CompStmtSave.size());
